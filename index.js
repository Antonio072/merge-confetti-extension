...
function runScript() {
    ...
    let isPullRequestSection = currentLocation.includes("pull")

    if (isPullRequestSection) {
        let isMergeConflictPage = currentLocation.includes("conflict")
        if (isMergeConflictPage) {
            const solveMergeConflictButton = document.querySelector('.tooltipped-nw')
            let alreadySubmitted = false
            solveMergeConflictButton.addEventListener('click', (event) => {
                if (!alreadySubmitted){
                    event.preventDefault()
                    jsConfetti.addConfetti()
                    alreadySubmitted = true
                    solveMergeConflictButton.click()
            ...
            setTimeout(() => {
                ...
                buttons = buttons.filter((el) => el.textContent.includes('Confirm'))
                let alreadySubmitted = false
                
                buttons.forEach((button) => {
                    button.addEventListener('click', (event) => {gi
    ...
    console.log("LOG: Current location: ", currentLocation)
}
